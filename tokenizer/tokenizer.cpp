        ...
        while (true) {
            ...
            switch (current_state) {
                ...
                case PLUS_SIGN_STATE: {
                    ...
                    // 回答：
                    // 首次进入while循环的时候，进行了第一次读字符，此时currentPos指向加号的后一个位置
                    // 之后break，第二次进入while循环，进入case PLUS_SIGN_STATE，第二次读字符，此时currentPos指向加号的后两个位置
                    // case PLUS_SIGN_STATE结束后currentPos应该指向加号的后一个位置
                    // 因此需要unreadLast回退
