...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
        ...
        out = prefix * std::stoi(next.value().GetValueString(),0,10);
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
        ...
        int32_t dx = getIndex(next.value().GetValueString());

		// 是否uninitialized
		if(isUninitializedVariable(next.value().GetValueString()))
        {
		    _uninitialized_vars.erase(next.value().GetValueString());
		    _vars.insert( std::pair< std::string, int32_t >(next.value().GetValueString(), dx));
        }
		...
		_instructions.emplace_back(Operation::STO, dx);
	...
	std::optional<CompilationError> Analyser::analyseFactor() {
		...
		switch (next.value().GetType()) {
			...
			case TokenType::IDENTIFIER:
            {
                ...
                _instructions.emplace_back(Operation::LOD, dx);
                break;
            }
		    ...
		    {
                int32_t out = prefix * std::stoi(next.value().GetValueString(), 0, 10);
